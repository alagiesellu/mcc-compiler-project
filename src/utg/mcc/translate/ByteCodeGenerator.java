...
public class ByteCodeGenerator extends ASTVisitor {
	...
	public Object visitExpressionStatement(ASTExpressionStatement expressionStatement, Object o) {
		...
		m.i();
		emt("\t\t; start expression");
		expressionStatement.expression.accept(this, null);
		ASTVariable variable = expressionStatement.variable;
		if (variable.entry.global) {
			emt("putstatic utg/mcc/bytecode/A/" + variable.entry.name + " " + variable.entry.type.signature());
		} else {
			emt("istore " + variable.entry.address + "\t\t; store " + variable.entry.name);
		}
		as(-1);
		emt("\t\t; end expression");
		m.d();
	...
	public Object visitCall(ASTCall call, Object o) {
		...
		emt("invokestatic utg/mcc/bytecode/A/" + call.entry.name + call.entry.type.signature());
	...
	public Object visitFunDeclaration(ASTFunDeclaration funDeclaration, Object o) {
		...
		actStack = 0;
		maxStack = 0;
		em(".method public static " + funDeclaration.name + funDeclaration.entry.type.signature());
	...
	public Object visitVarDeclaration(ASTVarDeclaration varDeclaration, Object o) {
		...
		if (varDeclaration.entry.global) {
			em(".field public static " + varDeclaration.entry.name + " " + varDeclaration.entry.type.signature());
		}
		m.d();
		m.verbose("< variable declaration");
		return null;
	...
	public Object visitVariable(ASTVariable variable, Object o) {
		...
		m.i();
		if (variable.entry.global) {
			emt("getstatic utg/mcc/bytecode/A/" + variable.entry.name + " " + variable.entry.type.signature());
		} else {
			emt("iload " + variable.entry.address + "\t\t; load " + variable.entry.name);
		}
		as(1);
		m.d();
		m.verbose("< variable");
	...
	public Object visitWhileStatement(ASTWhileStatement whileStatement, Object o) {
		...
		String lend = label();
		eml(lstart);
		whileStatement.expression.accept(this, null); // this will store expression true or false value on stack
		emt("ifeq " + lend);
		as(-1);
		emt("\t\t; while statement");
